...
MODEL_NAME = 'CLS_CAT_Adam'
SAVE_MODEL_DIR = 'Output_CLS'
NAME_DATASET = 'WESAD'
SUBJECT_ID_TEST = ['S9', 'S17', 'S6', 'S14', 'S3', 'S15', 'S11']
SUBJECT_ID_TEST = ['S10', 'S13', 'S16', 'S2', 'S4', 'S5', 'S7', 'S8']
...
class Model_CLS(nn.Module):
    ...
    def forward(self, eda_ft, hr_ft):
        ...
        cat_logit = torch.cat((eda_logit, hr_logit, com_logit), dim=1)
        cat_logit = self.final_cls(cat_logit)
        
        return eda_logit, hr_logit, com_logit, cat_logit
...
def train_epoch(model, optimizer, dataloader, loss_func):
    ...
    for i, (eda_ft, hr_ft, labels) in enumerate(dataloader):
        ...
        eda_logit, hr_logit, com_logit, cat_logit = model(eda_ft, hr_ft)
        
        loss_eda = loss_func(eda_logit, labels)
        loss_hr = loss_func(hr_logit, labels)
        loss_com = loss_func(com_logit, labels)
        loss_cat = loss_func(cat_logit, labels)
        loss_total = loss_eda + loss_hr + loss_com + loss_cat
        ...
        str_info += f'CAT Loss: {round(np.mean(cat_loss), 4)}\n'
    ...
    loss_dict['cat'] = np.mean(cat_loss)
...
def metric_to_info(metric_dict):
    ...
    info += '~~~ CAT RESULT~~~\n'
    info += f"Acc: {round(np.mean(metric_dict['cat']['acc']), 4)}"
    info += f" -- BAcc: {round(np.mean(metric_dict['cat']['bacc']), 4)}"
    info += f" -- F1: {round(np.mean(metric_dict['cat']['f1']), 4)}\n"
    return info
...
def loss_to_info(loss_dict):
    ...
    info += f" -- CAT Loss: {round(np.mean(loss_dict['cat']), 4)}"
    return info
...
def main(subject_id_test):
    ...
    for epoch in range(MAX_EPOCH):
        ...
        total_loss_val = 2 - metric_val_dict['cat']['f1'] - metric_val_dict['cat']['bacc']
